...
<?php if (have_comments() || comments_open()) : ?>
    <div id="comments" class="blog-post-comment postbox__comment-wrapper">
        ...
        <?php if (get_comments_number() >= 1): ?>
            <div class="post-comments mb-70 postbox__comment mb-95">
                <div class="postbox__comment-title-wrapper">
                    ...
                    <?php
                    // Compare the raw count, not the formatted string: at 1,000
                    // comments number_format_i18n() returns "1,000", which casts
                    // to 1 and picked the singular.
                    $comment_count = (int) get_comments_number();

                    if ($comment_count > 0) {
                        printf(
                            '<h2 class="postbox__comment-title">%s</h2>',
                            esc_html(
                                sprintf(
                                    /* translators: %s: number of comments. */
                                    _n('%s Comment', '%s Comments', $comment_count, 'portolite'),
        ...
        <?php if (get_comment_pages_count() > 1 && get_option('page_comments')): ?>
            <div class="comment-pagination mb-50 d-none">
                <nav id="comment-nav-below" class="comment-navigation">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="nav-previous "><?php previous_comments_link(esc_html__('← Older', 'portolite')); ?></div>
                        </div>
                        <div class="col-md-6">
                            <div class="nav-next "><?php next_comments_link(esc_html__('Newer →', 'portolite')); ?></div>
                        </div>
                    </div>
        ...
        // comment_form() merges `fields` with wp_parse_args(), a shallow merge, so
        // passing only author/email/url used to replace core's `cookies` field away
        // and silently drop the GDPR consent checkbox. Re-add it here.
        if (has_action('set_comment_cookies', 'wp_set_comment_cookies') && get_option('show_comments_cookies_opt_in')) {
            $consent = empty($commenter['comment_author_email']) ? '' : ' checked="checked"';

            $fields['cookies'] = '<div class="row"><div class="col-12"><p class="comment-form-cookies-consent">'
                . '<input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' /> '
                . '<label for="wp-comment-cookies-consent">' . esc_html__('Save my name, email, and website in this browser for the next time I comment.', 'portolite') . '</label>'
                . '</p></div></div>';
        }
        ...
        ',
            'submit_button'    => '<div class="col-xl-12"><button class="thm-btn contact-one__btn" type="submit">' . esc_html__('Post Comment', 'portolite') . ' </button></div>',
            ...
            'id_form'            => 'commentform',
            'id_submit'          => 'submit',
            'class_submit'       => 'thm-btn contact-one__btn',
