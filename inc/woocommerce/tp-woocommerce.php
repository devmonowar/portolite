...
*************************************************/
function portolite_shipping_progress_bar() {
...
if(get_theme_mod( 'shipping_progress_bar_location_card_page',0) == '1'){
    add_action( 'woocommerce_before_cart_table',  'portolite_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_mini_cart',0) == '1'){
    add_action( 'woocommerce_before_mini_cart_contents', 'portolite_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_checkout',0) == '1'){
    add_action( 'woocommerce_checkout_before_customer_details', 'portolite_shipping_progress_bar' );
}
...
function portolite_sale_percentage(){
   global $product;
   $output = '';
...
if ( ! function_exists( 'portolite_header_add_to_cart_fragment' ) ) {
...
add_filter( 'woocommerce_add_to_cart_fragments', 'portolite_header_add_to_cart_fragment' );

// portolite_shopping_cart
function portolite_shopping_cart(){
    ob_start();
    ?>
            ...
            <div class="cartmini__top-wrapper">
               <div class="cartmini__top p-relative">
                  <div class="cartmini__title">
                     <h4>S<?php print esc_html__('Shopping cart','portolite'); ?></h4>
                  </div>
...
if( !function_exists('portolite_content_product_grid') ) {
    function portolite_content_product_grid( ) {
        global $product;
        global $post;
        global $woocommerce;
            ...
            <?php if( has_post_thumbnail() ) : ?>
              <div class="product__thumb w-img p-relative fix">
                 ...
                 <?php if( function_exists( 'woosw_init' )) : ?>
                    <div class="product-action-btn product-add-wishlist-btn">
                         ...
                         <span class="product-action-tooltip"><?php echo esc_html__('Add To Wishlist','portolite'); ?></span>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="product-action-btn">
                       ...
                       <span class="product-action-tooltip"><?php echo esc_html__('Quick view','portolite'); ?></span>
                    </div>
                    <?php endif; ?>
                     <?php if( function_exists( 'woosc_init' )) : ?>
                    <div class="product-action-btn">
                       <?php echo do_shortcode('[woosc]');?>                                       
                       <span class="product-action-tooltip"> <?php echo esc_html__('Add To Compare','portolite'); ?></span>
                    </div>
                    <?php endif; ?>
                 </div>
              ...
              <div class="product__content">
                 ...
                 <div class="product__rating d-flex mb-10">
                    <?php echo portolite_kses($rating); ?> 
                 </div>
                 <?php endif; ?>
...
add_action( 'woocommerce_before_shop_loop_item', 'portolite_content_product_grid', 10 );


// product-content
if( !function_exists('portolite_content_product_list') ) {
    function portolite_content_product_list( ) {
        global $product;
        global $post;
        global $woocommerce;
        $rating = wc_get_rating_html($product->get_average_rating());
...
add_action( 'woocommerce_before_shop_loop_item_list', 'portolite_content_product_list', 10 );

// smart quickview
add_filter( 'woosq_button_html', 'gportolite_woosq_button_html', 10, 2 );
function gportolite_woosq_button_html( $output , $prodid ) {
    ...
    global $product;
        ...
        if ( $product ) {
         ...
         if( $product->is_type( 'simple' ) ){
            $btntext = esc_html__("Add to Cart",'portolite');
         } elseif( $product->is_type( 'variable' ) ){
            $btntext = esc_html__("Select Options",'portolite');
         } elseif( $product->is_type( 'external' ) ){
            $btntext = esc_html__("Buy Now",'portolite');
         } elseif( $product->is_type( 'grouped' ) ){
            $btntext = esc_html__("View Products",'portolite');
         }
         else{
            $btntext = esc_html__("Add to Cart",'portolite');
         } 
        ...
        $args = wp_parse_args(
            ...
            apply_filters(
                ...
                array(
                    ...
                    'before'      => '',
                    'after'       => '',
                    'home'        => _x( 'Home', 'breadcrumb', 'portolite' ),
                )
            )
        );
